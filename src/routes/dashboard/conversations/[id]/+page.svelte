<script>
    ...
    async function sendMessage() {
        ...
        let message = { message:messageText, messageId:(Date.now()+Math.floor(Math.random() * 10000)).toString(), conversation:conversation.id, sender:user, receiver:chattingWith, files:encodedFiles };
