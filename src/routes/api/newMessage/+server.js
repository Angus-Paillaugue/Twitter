...
export async function POST({ locals, request }) {
    ...
    await messagesRef.insertOne({ conversation:id, id:message.messageId, sender:locals.user.username, receiver:usernames.users.filter(username => username !== locals.user.username)[0], message, date:new Date(), seen:false });
